      ...
      <div className="mt-5 mx-5 pb-5 border-b-2 border-black flex flex-row justify-between items-baseline">
        ...
        <div>
          <button onClick={() =>  setIsModalOpen(true)}
            className="bg-[#75ba75] rounded-2xl  px-7 py-1 text-white font-size"
          >
            Add Links
          </button>
        </div>
                 ...
                 <tbody>
                     {links.map((link, index) => (
                      <tr key={index} className={`${index % 2 === 0 ? 'bg-[#7EBA7E]' : 'bg-[#d2f0d2]'}`} onSubmit={addLinks}>
                          ...
                          <td className="text-center p-2 overflow-hidden overflow-wrap break-word">{link.class_description}</td>
